...
class FlickrUser : NSObject {
    ...
    override init() {
      ...
      func bboxString(latitude: Double, longitude: Double) -> String {
        ...
        let minimumLon = max(longitude - FlickrUser.SearchBBoxHalfWidth, FlickrUser.SearchLonRange.0)
        let minimumLat = max(latitude  - FlickrUser.SearchBBoxHalfHeight, FlickrUser.SearchLatRange.0)
        let maximumLon = min(longitude + FlickrUser.SearchBBoxHalfWidth, FlickrUser.SearchLonRange.1)
        let maximumLat = min(latitude  + FlickrUser.SearchBBoxHalfHeight, FlickrUser.SearchLatRange.1)
    ...
    private func tmdbURLFromParameters(_ parameters: [String:AnyObject]) -> URL {
        ...
        components.scheme = FlickrUser.Constants.ApiScheme
        components.host = FlickrUser.Constants.ApiHost
        components.path = FlickrUser.Constants.ApiPath
    ...
    class func sharedInstance() -> FlickrUser {
        struct Singleton {
            static var sharedInstance = FlickrUser()
        }
